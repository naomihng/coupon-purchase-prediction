...
pref_locations <- pref_locations[,c("LATITUDE", "LONGITUDE", "en_PREF_NAME")]

names(pref_locations) <- c("coupon_LATITUDE", "coupon_LONGITUDE", "en_PREF_NAME")
dat <- merge(dat, pref_locations, by.x = "coupon_pref", by.y = "en_PREF_NAME", all.x = TRUE)

names(pref_locations) <- c("user_LATITUDE", "user_LONGITUDE", "en_PREF_NAME")
dat <- merge(dat, pref_locations, by.x = "user_pref", by.y = "en_PREF_NAME", all.x = TRUE)
...
dat$USABLE_DATE_Weekday[is.na(dat$USABLE_DATE_Weekend)] <- 1
dat$USABLE_DATE_HOLIDAY<-ifelse(dat$USABLE_DATE_HOLIDAY!=0,1,0)
dat$USABLE_DATE_HOLIDAY[is.na(dat$USABLE_DATE_Weekday)] <- 1
dat$USABLE_DATE_BEFORE_HOLIDAY<-ifelse(dat$USABLE_DATE_BEFORE_HOLIDAY!=0,1,0)
dat$USABLE_DATE_BEFORE_HOLIDAY[is.na(dat$USABLE_DATE_Weekday)] <- 1
