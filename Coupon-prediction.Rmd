...
plot(nb, col="red")
auc_pr(train$purchase,nb.prob$`1`)
```
...
##model evaluation 
```{r}
plot(rpart_tree, col="green", add= TRUE)
plot(nb, col= "red", dd = TRUE)
plot(knn, col = "blue",add = TRUE)
plot(log, col = "orange", add = TRUE)
legend('bottomleft', c("rpart_tree","NB","KNN","log") , 
       lty=1, col=c('green','red', 'blue',"orange"), bty='n', cex=.75)
```

Best Model: 
```{r}
#best.pred <- predict(######,newdata = test)
#confusionMatrix(best.pred, test$purchase)
```
```{r}
