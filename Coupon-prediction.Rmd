...
ctrl <- trainControl(method = "repeatedcv", number = 10, savePredictions = TRUE, classProbs = TRUE)
set.seed(4217)
rpartTune <- train(purchase ~ ., data = train, method = "rpart", trControl = ctrl, parms=list(split='information'))
rpartTune$finalModel

plot(rpartTune$finalModel)

rpartPred <- predict(rpartTune, train)
summary(rpartPred)
rpartPred
rpartProbs <- predict(rpartTune, train, type = "prob")
summary(rpartProbs)
...
library(ROCR)
pr <- prediction(rpartProbs[,2], train$purchase)
...
# confusionMatrix(rpartProbs$X2, train$purchase)
```
