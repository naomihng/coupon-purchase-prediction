...
#Business Case
Companies like Groupon help companies to generate demand by offering coupons to the consumers, usually offering them at a discounted rate. Consumers /users information could be collected and analyses to recommend coupons that the users are likely to use. This analysis on coupon prediction has a wide-ranging application as: 
1)	There is a cost to recommending a coupon  
a.	Cost of emailing the consumers
b.	Opportunity cost of using the space on the app for other advertisement 
...
1)	Understand what are some variables that incentives people to purchase the coupons. 
2)	Predict how likely is someone going to purchase given that they view. 
Using the prediction, we can understand how likely will they buy the coupon if they view it. Together with another model to understand the probability each user view the coupons, we can recommend coupons to the users. This aims to reduce the cost of emailing the unsuitable coupons to the customers and earning greater commission as users used our recommended coupons. 
...
Changing of Japanese names to english names 

```{r}
## adapted from andreas https://www.kaggle.com/anguyen/coupon-purchase-prediction/translate-everything-to-english-using-r 
dir <- './data/'
...
#Retrieving Data and subsetting data 
...
b.	Reason: Precision recall aims to measure how efficient and precise the model, and measure whether we are recommending the suitable coupons to the customers and out of all the suitable coupons for the customers, how many are we recommending 
...
## Classification Tree
Classification tree was used as it provide a easy method to understand purchasing decision since the tree is simple to understand. 
```{r}
...
##Logistic Regression
Lasso Regression are used based on the objective to minimised the variance and perform regularisation. 
...
##KNN
KNN was not used in the end due to high number of observation which requires to calculate large number of distance for prediction. However, This coule be one of a feasible model for prediction as the observations that are similar have similar purchasing decision.  
...
Applying it the best model to the test dataset
...
From the intersection point, Theshold of 0.25 might be the to get the best precision and recall 
