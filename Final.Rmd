...
FORK and adapted from andreas https://www.kaggle.com/anguyen/coupon-purchase-prediction/translate-everything-to-english-using-r 
```{r}
dir <- './data/'
...
names(coupon_list_train)[names(coupon_list_train)=="en_ken"] <- "coupon_pref"
...
prefecture_locations=merge(prefecture_locations,trans,by.x="ï..PREF_NAME",by.y="jp",all.x=T)
...
There is location information (ken, small area name, large area name) for each coupon in this data set.
```{r}
head(list_train)
...
From the intersection point, a threshold of 0.25 might get the best precision and recall {EDITED}
